...
resource "kubernetes_secret" "external_secrets_aws_access_key" {
  metadata {
    name      = "external-secrets-aws-access-key"
    namespace = kubernetes_namespace.external_secrets.metadata[0].name
  }
  type = "Opaque"
  data = {
    aws_iam_access_key = var.external_secrets_aws_iam_access_key
    aws_iam_secret_key = var.external_secrets_aws_iam_secret_key
  }
}

resource "kubectl_manifest" "aws_nonprod_secretstore" {
    ...
    spec = {
      provider = {
        aws = {
          ...
          region  = "us-east-1"
          role    = "arn:aws:iam::744848298718:role/DevZoneSecretsManager"
          auth = {
            secretRef = {
              accessKeyIDSecretRef = {
                name      = kubernetes_secret.external_secrets_aws_access_key.metadata[0].name
